...
void CurlFtp::listFiles(const string &strDir, FileInfoList &fileList)
{
    ...
    // modified by liubojun
    if (m_bRecordLatestTime)
    {
        m_pRecordLatestTime(strDBPath, QString::fromLocal8Bit(strDir.c_str()), iLatestTime);
    ...
    if (m_bRecordLatestTime)
    {
        ...
        {
            m_pRecordLatestTime(strDBPath, QString::fromLocal8Bit(strDir.c_str()), nMdfTime);
        }
    }
...
QString CurlFtp::parseMlsdInfo(const QString &info, FileInfoList &fileList, QStringList &dirList, const QString & iLatestTime, bool bFtpSupportMSDL)
{
    ...
    for (int i=0; i<lstLines.size(); ++i)
    {
        ...
        if (oneInfo.nType == 1)
        {
            ...
            // 必须大于设置的时间范围才收集
            // modified by liubojun
            if (m_iCol_timerange != -1)
            {
                // 分钟转秒
                if (nMdfyTime < QDateTime::currentDateTime().addSecs(-m_iCol_timerange * 60).toString("yyyyMMddhhmmss"))
                {
                    continue;
                }
            }
