...
typedef void (*FunctorRecordLatestTime)(const QString &, const QString &, QString &);

//////////////////////////////////////////////////////////////////////////

class DLL_EXPORT_CLASS_DECL CurlFtp : public QObject
{
    Q_OBJECT
public:
    ...
    /**
    * @brief 解析LIST命令返回列表信息，并封装到fileList中
    * @param  const QString &info LIST命令返回的字符串内容
    * @param  FileInfoList &fileList 解析LIST中返回的文件列表信息
    * @param  QStringList &dirList 目录中的子目录列表
    * @param  const QString & iLatestTime 目录最新处理时间
    * @param  bool bFtpSupportMSDL 是否支持MLIST命令
    * @return QString：
    */
...
private:
    ...
    // 记录最后修改时间回调函数
    FunctorRecordLatestTime m_pRecordLatestTime;

    // 是否记录最后修改时间
    bool m_bRecordLatestTime;

    // 收集时间范围，当设置为-1时，表示收集所有时间的文件
    int m_iCol_timerange;
};
