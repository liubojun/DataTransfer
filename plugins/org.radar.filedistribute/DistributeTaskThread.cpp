...
void DistributeTaskThread::run()
{
    ...
    if (1 == m_fileInfo.m_iCollectWay)	// ftp
    {
        ...
        {
            QSLOG_ERROR(QStringLiteral("从Ftp获取文件失败。"));
    ...
    else if (0 == m_fileInfo.m_iCollectWay)	// 共享目录
    {
        ...
        if (0 != m_pFtp->downloadFile(url, usrPwd, &fileData))
        {
            for (int i=0; i<m_fileInfo.m_strUsers.size(); ++i)
            {
                ...
                QSLOG_ERROR(QStringLiteral("从目录获取文件失败。"));
    ...
    for (int i=0; i<m_fileInfo.m_strUsers.size(); ++i)
    {
        ...
        if (!CrypCurtFile(tmpFilePath, userInfo.m_bCompress, userInfo.m_bEncrypt, nIndex, strFileFullPath))
        {
...
// 加密压缩落盘文件
bool DistributeTaskThread::CrypCurtFile(const QString &srcFilePath, bool bComp, bool bCrypt, int &nIndex, QString &dstFilePath)
{
    if (m_pFileCrypt == NULL)
    {
        return false;
