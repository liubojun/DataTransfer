...
FileDistributeThread::FileDistributeThread(void *parent)
{
    ...
    QObject::connect(&g_SignalObject, SIGNAL(progress(int, qint64, qint64, qint64, qint64)), this, SLOT(onFtpProgress(int, qint64, qint64, qint64, qint64)));
...
//     }
    ...
    for (int i=0; i<m_nMaxThreadCount; ++i)
    {
        ...
        m_vecThread.append(pThread);
        m_vecWorkState.append(false);
        m_vecThread[i]->start();
    }
}
...
//     foreach (QSharedPointer<DistributeTaskThread> pThread, m_mapIdThread)
//     {
//         if (!pThread.isNull())
//         {
//             pThread->quit();
...
void FileDistributeThread::AddSendTask(const TransferSet & tSet)
{
    ...
    for (int i=0; i<tSet.fileList.size(); ++i)
    {
        ...
        QSharedPointer<DistributeTaskThread> task = QSharedPointer<DistributeTaskThread>(new DistributeTaskThread(this, tInfo));
        QElapsedTimer t;
        while (m_nCurThreadCount >= m_nMaxThreadCount)
        {
            t.start();
            if (!m_bRun)
            {
        ...
        for (int i=0; i<m_nMaxThreadCount; ++i)
        {
            if (!m_vecWorkState[i])
            {
                task->moveToThread(m_vecThread[i].data());
                task->m_threadId = i;
                m_vecWorkState[i] = true;
                break;
            }
        }

        connect(task.data(), SIGNAL(taskFinished(int,int)), this, SLOT(onTaskFinished(int,int)), Qt::QueuedConnection);
...
void FileDistributeThread::onTaskFinished(int id, int threadId)
{
    QMutexLocker autoLocker(&m_taskMutex);
    ...
    if (threadId >= 0 && threadId < m_nMaxThreadCount)
    {
        m_vecWorkState[threadId] = false;
    }
}

void FileDistributeThread::onFtpProgress(int id, qint64 dltotal, qint64 dlnow, qint64 ultotal, qint64 ulnow)
{
    if (m_mapIdThread.contains(id))
    {
        ...
        if (dltotal != 0)
        {
            fileInfo.m_nSpeed = (dlnow - fileInfo.m_getDone) / m_pFtp->m_nPgtime;
        ...
        {
            fileInfo.m_nSpeed = (ulnow - fileInfo.m_putDone) / m_pFtp->m_nPgtime;
