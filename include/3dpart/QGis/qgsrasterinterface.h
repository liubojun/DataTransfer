...
public:
    ...
    {
        if ( mInput ) return mInput->srcDataType( bandNo );
        else return QGis::UnknownDataType;
    };
    ...
    {
        if ( mInput ) return mInput->extent();
        else return QgsRectangle();
    }
    ...
    {
        if ( mInput ) return mInput->xBlockSize();
        else return 0;
    }
    virtual int yBlockSize() const
    {
        if ( mInput ) return mInput->yBlockSize();
        else return 0;
    }
    ...
    {
        if ( mInput ) return mInput->xSize();
        else return 0;
    }
    virtual int ySize() const
    {
        if ( mInput ) return mInput->ySize();
        else return 0;
    }
    ...
    // modified by liubojun, @20151109 添加渲染上下文对象的参数传递
    virtual QgsRasterBlock *block( int bandNo, const QgsRectangle &extent, int width, int height, QgsRenderContext *renderCxt = NULL ) = 0;
