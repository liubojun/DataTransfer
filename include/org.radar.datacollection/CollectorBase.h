...
public:
     ...
     * @brief  默认构造函数
     */
    CollectorBase(CollectManager *pManager, QWaitCondition &in_oCond, QMutex &in_oLocker, int &in_iLogsize, QObject *pParent=NULL);
...
protected:
    ...
    LogInterface *m_pLog;
    IFileTransfer *m_pTransferDlg;
    //QSharedPointer<IRadarBaseDataParse> m_pBaseParser;
    QSharedPointer<IMakePath> m_pMakePath;
    IDataParse *m_pDataParseServ;	///< 数据解析服务
...
public:
     ...
     */
    //bool IsHistyFile(FileInfo &fileInfo);

    CollectManager *m_pManager;
